...
import CreateRequestAccessModal from './create-modal';
  ...
  return (
    <div className='w-full'>
      <div className='flex flex-col mb-6 space-y-4 lg:justify-between lg:flex-row lg:items-center lg:space-y-0 lg:space-x-4'>
        <Input
          autoFocus
          placeholder='Filter Request Access by ID'
          value={(table.getColumn('request_access_id')?.getFilterValue() as string) ?? ''}
          onChange={(event) => table.getColumn('request_access_id')?.setFilterValue(event.target.value)}
        ...
        <div className='flex flex-col items-center w-full space-x-2 space-y-4 lg:w-auto lg:flex-row lg:space-y-0 lg:space-x-4'>
          ...
          {user && ['WashingWorker', 'IroningWorker', 'PackingWorker'].includes(user.role) && (
...
interface RequestAccessTableProps {
  //
}

const RequestAccessTable: React.FC<RequestAccessTableProps> = ({ ...props }) => {
  ...
  const { data, error, isLoading } = useRequestAccess(filter, pagination, sorting);
  ...
  return (
    <DataTable
      columns={columns}
      data={data.data.requestAccesses}
...
export default RequestAccessTable;
