...
import { useOutletEmployees } from '@/hooks/use-outlet-employees';
    ...
    <div className='w-full'>
      <div className='flex flex-col mb-6 space-y-4 lg:justify-between lg:flex-row lg:items-center lg:space-y-0 lg:space-x-4'>
        ...
        <div className='flex flex-col items-center w-full space-x-2 space-y-4 lg:w-auto lg:flex-row lg:space-y-0 lg:space-x-4'>
          ...
          <CreateEmployeeModal outlet_id={outlet_id} />
        </div>
      </div>
...
const EmployeesTable: React.FC<EmployeesTableProps> = ({ outlet_id, ...props }) => {
  ...
  const { data, error, isLoading } = useOutletEmployees(outlet_id, filter, pagination, sorting);
